#[allow(unused_field)]
module cifarm::rambutan_collection {
    // ===== Imports =====
    ...
    public struct RAMBUTAN_COLLECTION has drop {}
    ...
    fun init(otw: RAMBUTAN_COLLECTION, ctx: &mut TxContext) {
        // Create collection
        let (treasury_cap, collection_metadata) = nft_collection::create_collection(
            otw,
            string::from_ascii(ascii::string(b"Rambutan Collection")),
    ...
    public fun update_name(
        self: &mut NFT<RAMBUTAN_COLLECTION>,
        name: string::String,
    ) {
